...
func set_grapheme_container(grapheme: Object) -> void:
	...
	self.identify_databse_res.show_debug();
	print(self.identify_databse_res.selected_grapheme);
	print(self.identify_databse_res.find_selected_item(self.grapheme_item));
	if(self.identify_databse_res.find_selected_item(self.grapheme_item)):
		$Select.pressed = true;
